...
public class Config {
  ...
  private static class ConfigScreen extends Screen {
    ...
    private static final Text VALUE_ENABLE =
        Text.translatable("${MOD_ID}.config.value.enable").formatted(Formatting.GREEN);
    private static final Text VALUE_DISABLE =
        Text.translatable("${MOD_ID}.config.value.disable").formatted(Formatting.DARK_RED);
    ...
    public static Text getValueText(String key) {
      ...
      return Text.translatable(
          "%s: %s",
          Text.translatable("${MOD_ID}.config." + key), bool ? VALUE_ENABLE : VALUE_DISABLE);
    }
      ...
      for (String key : DEFAULT.keySet()) {
        this.addDrawableChild(
            ButtonWidget.builder(
                ...
                .dimensions(this.width / 2 - 150, i, 300, 20)
                .build());
