...
public class Render {
        ...
        .mulPose(
            mc.gameRenderer.getProjectionMatrix(
                ((GameRendererAccessor) mc.gameRenderer)
                    .callGetFov(mc.gameRenderer.getMainCamera(), partialTicks, true)));
  }
  ...
  public static void distortion(MatrixStack mat, float tickDelta) {
    ...
    float f =
        Mth.lerp(tickDelta, mc.player.oPortalEffectIntensity, mc.player.portalEffectIntensity)
            * g
            * g;
    if (f > 0.0F) {
      int i = mc.player.hasEffect(MobEffects.NAUSEA) ? 7 : 20;
      float f1 = 5.0F / (f * f + 5.0F) - f * 0.04F;
      f1 = f1 * f1;
      mat.rotate(
          (mc.player.tickCount + tickDelta) * i,
          0.0F,
