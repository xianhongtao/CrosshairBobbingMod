...
public class Render {
    ...
    public static void updateCameraMatrix(MatrixStack mat, float partialTicks) {
        ...
        mat.pose().multiplyPositionMatrix(mc.gameRenderer.getBasicProjectionMatrix(((GameRendererAccessor) mc.gameRenderer).callGetFov(mc.gameRenderer.getCamera(), partialTicks, true)));
    }
    ...
    public static void distortion(MatrixStack mat, float tickDelta) {
        ...
        float f = MathHelper.lerp(tickDelta, mc.player.lastNauseaIntensity, mc.player.nauseaIntensity) * g * g;
        if (f > 0.0F) {
           int i = mc.player.hasStatusEffect(StatusEffects.NAUSEA) ? 7 : 20;
           float f1 = 5.0F / (f * f + 5.0F) - f * 0.04F;
           f1 = f1 * f1;
           mat.rotate((mc.player.age + tickDelta) * i, 0.0F, MathHelper.SQUARE_ROOT_OF_TWO / 2.0F, MathHelper.SQUARE_ROOT_OF_TWO / 2.0F);
