...
public class Player {
  ...
  public static float spinProgress(float partialTicks) {
    ...
    if (mc.player.isAutoSpinAttack()) {
      return Mth.clamp(
          (20 - ((LivingEntityAccessor) mc.player).getAutoSpinAttackTicks() + partialTicks) / 20.0f,
          0.0f,
          1.0f);
    }
  ...
  public static float sneakProgress(float partialTicks) {
    ...
    if (camera.entity() != null) {
      ...
      if ((entity.getPose() == Pose.STANDING && lastPose == Pose.CROUCHING)
  ...
  public static float swingProgress(float partialTicks) {
    ...
    if (camera.entity() == player && player.swinging) {
  ...
  public static float usingItemProgress(float partialTicks) {
    ...
    if (camera.entity() == player && player.isUsingItem()) {
      ItemStack item = player.getUseItem();
      float f =
          item.getUseDuration(player) - (player.getUseItemRemainingTicks() - partialTicks + 1.0f);
      ItemUseAnimation anim = item.getUseAnimation();

      if (anim == ItemUseAnimation.BOW) {
        return Math.min(f / 20.0f, 1.0f);
      } else if (anim == ItemUseAnimation.SPEAR || anim == ItemUseAnimation.TRIDENT) {
  ...
  public static ItemType getUsingType() {
    ...
    if (camera.entity() == player && player.isUsingItem()) {
      ...
      } else if (anim == ItemUseAnimation.CROSSBOW && !CrossbowItem.isCharged(item)) {
