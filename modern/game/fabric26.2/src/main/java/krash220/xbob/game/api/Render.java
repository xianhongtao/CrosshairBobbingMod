...
public class Render {
  ...
  /**
   * 相机 bob：与 26.2 原版 {@code GameRenderer.bobHurt/bobView} 相同的数学， 但自实现（不调用渲染专用的私有方法，避免其操作
   * RenderSystem 全局状态）。
   */
  public static void bobView(MatrixStack mat, float partialTicks) {
    Minecraft mc = Minecraft.getInstance();
    CameraEntityRenderState entity =
        mc.gameRenderer.gameRenderState().levelRenderState.cameraRenderState.entityRenderState;
