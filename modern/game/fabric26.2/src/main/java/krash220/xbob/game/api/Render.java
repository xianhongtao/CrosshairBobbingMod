...
public class Render {
    ...
    public static boolean isDebugCrosshair() {
        ...
        return mc.getDebugOverlay().showDebugScreen() && !mc.player.isReducedDebugInfo() && !mc.options.reducedDebugInfo().get();
    }

    /**
     * 相机 bob：与 26.2 原版 {@code GameRenderer.bobHurt/bobView} 相同的数学，
     * 但自实现（不调用渲染专用的私有方法，避免其操作 RenderSystem 全局状态）。
     */
    public static void bobView(MatrixStack mat, float partialTicks) {
        Minecraft mc = Minecraft.getInstance();
        CameraEntityRenderState entity = mc.gameRenderer.gameRenderState().levelRenderState.cameraRenderState.entityRenderState;

        if (!entity.isPlayer) {
            return;
        }

        OptionsRenderState options = mc.gameRenderer.gameRenderState().optionsRenderState;
    ...
    private static void bobHurt(MatrixStack mat, CameraEntityRenderState entity, OptionsRenderState options) {
    ...
    private static void bobWalk(MatrixStack mat, CameraEntityRenderState entity) {
        ...
        mat.translate(Mth.sin(walkDist * Mth.PI) * bob * 0.5, -Math.abs(Mth.cos(walkDist * Mth.PI) * bob), 0.0);
    ...
    public static void updateCameraMatrix(MatrixStack mat, float partialTicks) {
        ...
        mat.pose().mulPose(mc.gameRenderer.gameRenderState().levelRenderState.cameraRenderState.projectionMatrix);
    }
