...
public class Config {
  ...
  private static File configFile() {
    return new File(FabricLoader.getInstance().getConfigDirectory(), "${MOD_ID}.json");
  }
  ...
  public static class ModMenu implements ModMenuApi {
    ...
    public ConfigScreenFactory<?> getModConfigScreenFactory() {
      return previous ->
          new ConfigScreen(Minecraft.getInstance(), Minecraft.getInstance().font, previous);
    }
  }

  private static class ConfigScreen extends Screen {

    private static final Component VALUE_ENABLE =
        Component.translatable("${MOD_ID}.config.value.enable").withStyle(ChatFormatting.GREEN);
    private static final Component VALUE_DISABLE =
        Component.translatable("${MOD_ID}.config.value.disable").withStyle(ChatFormatting.DARK_RED);
    ...
    public static Component getValueText(String key) {
      ...
      return Component.translatable(
          "%s: %s",
          Component.translatable("${MOD_ID}.config." + key), bool ? VALUE_ENABLE : VALUE_DISABLE);
    }

    private final Screen previous;

    protected ConfigScreen(Minecraft minecraft, Font font, Screen previous) {
      super(minecraft, font, Component.translatable("${MOD_ID}.config.title"));
      ...
      for (String key : DEFAULT.keySet()) {
        this.addRenderableWidget(
            Button.builder(
                ...
                .bounds(this.width / 2 - 150, i, 300, 20)
                .build());
